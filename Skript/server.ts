...
import { ladeEinstellungen, speichereEinstellungen, type Einstellungen } from './config/einstellungen'
...
import type { Beruf, Schueler, NoteEintrag } from './types'
...
function parseSchuelerFromBody(body: Record<string, unknown>): { schueler: Schueler; beruf: Beruf; halbjahre: string[]; halbjahrStunden?: Record<string, Record<string, number>>; lfStundenOverrides?: Map<string, number> } {
  ...
  if (rawAllg) {
    for (const [fach, noten] of Object.entries(rawAllg)) {
      allgNoten.set(fach, noten.map(n => ({ note: n && n > 0 ? n : null, lehrer: '' })))
    }
  }
  ...
  const lfStundenOverrides = lfOverrides ? new Map(Object.entries(lfOverrides).map(([k, v]) => [k, Number(v)])) : undefined
...
export async function startServer(port = 3000) {
      ...
      '/api/pdf': {
        POST: async (req) => {
          try {
            ...
            const parts = [schueler.nachname || 'Unbekannt', schueler.vorname, schueler.klasse, austritt].filter(Boolean).join('_')
      ...
      '/api/templates': {
        ...
        POST: async (req) => {
          ...
          if (body.type === 'stunden') {
            store.stundenTemplates.push({ id, name: body.name, data: body.data as TemplateStore['stundenTemplates'][0]['data'] })
          } else {
            store.komplettVorlagen.push({ id, name: body.name, data: body.data as TemplateStore['komplettVorlagen'][0]['data'] })
          }
  ...
  const open = process.platform === 'linux' ? 'xdg-open'
    : process.platform === 'darwin' ? 'open' : 'start'
